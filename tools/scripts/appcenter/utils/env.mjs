...
const env = cleanEnv($.env, {
  ...
  APPCENTER_SOURCE_DIRECTORY: str({
    desc: 'The directory where the source code is located',
  }),
  APPCENTER_OUTPUT_DIRECTORY: str({
    desc: 'The directory where the build output is located',
  }),
  APPCENTER_CURRENT_PLATFORM: str({
    choices: ['android', 'ios'],
    desc: 'The platform the app is being built for',
  }),
  APPCENTER_REACTNATIVE_PACKAGE: str({
    desc: 'The path to the React Native package.json file',
  }),
  APPCENTER_NX_PROJECT: str({
    desc: 'The name of the Nx project containing the React Native app',
  }),
  APPCENTER_NX_PROJECT_DIR: str({
    desc: 'The directory of the Nx project containing the React Native app',
  }),
  ...
  ...($.env.APPCENTER_CURRENT_PLATFORM === 'ios'
    ...
    ? {
        APPCENTER_ANDROID_VARIANT: str({
          desc: 'The name of the Android variant',
        }),
        APPCENTER_ANDROID_MODULE: str({
          desc: 'The name of the Android module',
        }),
      }
    : {}),
});
